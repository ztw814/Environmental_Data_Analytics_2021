 ...
 * Spatial data structures: sf, sfc (column of geometry), sfg (geometry of one specific row - coordinates plus projection info on how to place it on a map), geometry (coordinates only) objects
...
st_crs(huc8_sf)
st_crs(huc8_sf) == st_crs(counties_sf) #FALSE so different crs
...
mapview(huc8_sf, col.regions = "purple") +
  mapview(huc8_sf_Neuse, col.regions = "orange")
...
wind_energy <- read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv", 
  ...
  group_by(HUC_8) %>% 
  summarize(wind_sum = sum(AvgWindEnergy))
                          

#Join to HUC_8 features
huc8_sf_wind <- huc8_sf %>% 
  left_join(wind_energy, by = c("HUC_8"))



#View the outputs
mapview(huc8_sf_wind, zcol = "wind_sum")
