...
# matrix filtering using matrix notation
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #select all instances of Paul Lake or Peter Lake
...
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #cleaner way to filter out multiple values w/ a string
...
NTL.phys.data.19901999 <- filter(NTL.phys.data, year4 >1989, year4 <2000)
tail(NTL.phys.data.19901999)
```
...
NTL.phys.data.PeterPaul1 <- mutate(NTL.phys.data.PeterPaul1, month = month(sampledate)) #function "month" from lubridate
...
# create an interval object with two values from the dataset
# first and last from dplyr package
