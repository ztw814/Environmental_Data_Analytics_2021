...
# Correlation
cor.test(PeterPaul.chem.nutrients$irradianceWater, PeterPaul.chem.nutrients$depth) #negative correlation bt variables (-0.55)
```
...
1. **Residuals vs. Fitted.** The value predicted by the line of best fit is the fitted value, and the residual is the distance of that actual value from the predicted value. By definition, there will be a balance of positive and negative residuals. Watch for drastic asymmetry from side to side or a marked departure from zero for the red line - these are signs of a poor model fit. Want the line to be as flat as possible and for symmetry of point distribution around line.
...
3. **Scale-Location.** Similar to the residuals vs. fitted graph, this will graph the squared standardized residuals by the fitted values. Want symmetry w/ respect to red line.
...
par(mfrow = c(2,2), mar=c(4,4,4,4)) #mar sets up size of plots within mfrow 2x2 grid
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) +
  geom_point() 
print(irradiancebydepth) #shows that there is clearly not a linear relationship bt depth and irradiance
```
...
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth) #testing if log transformation has a linear relationship w/ depth; must filter out zeros to do so
...
irradiancebydepth2 <- 
  ...
  scale_y_log10() + #log transform y bc y = irradiance
  geom_point() 
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit()
PeterPaulCorr <- cor(PeterPaulnutrients) #table of correlations bt variables in PeterPaulnutrients
corrplot(PeterPaulCorr, method = "ellipse") #wider ovals mean less correlation
corrplot.mixed(PeterPaulCorr, upper = "ellipse") #mix of correlation numbers and ellipses
# high correlation indicates variables that you might want to include in the model
```
...
# lower AIC is better
step(TPAIC) #shows what the AIC if you removed each variable or if you removed <none> of the variables; also gives coefficients for best model
