...
str(USGS.flow.data) # look at structure of data
dim(USGS.flow.data)
...
ggplot(na.omit(USGS.flow.data), aes(x = discharge.mean.approval)) +
  geom_bar() #omits all rows w/ an NA value
```
...
#changing number of bins (bars) by varying their width
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

#changing number of bins by selecting a number
#generally between 5-20 bins is good
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

#force x-axis to cut off at 500
...
#defining color according to a categorical variable
ggplot(USGS.flow.data) +
...
#slicing data: showing the variance in discharge.mean across different slices of gage.height.mean
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#violin plot: shows both range of values as well as distribution at different values (width is relative to number of data points at a particular value)
ggplot(USGS.flow.data) +
