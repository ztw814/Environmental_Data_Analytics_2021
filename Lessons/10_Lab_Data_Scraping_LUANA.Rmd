...
```{r}
#Apply the API key
#census_api_key("4704da71b850b6b6ccf071c0dd17b40741706cbcf", install=TRUE)
...
#Plot population for NC counties
...
the_df <- data.frame(
  ...
  capacity_MW = the_capacity,
  netgeneration_MWh = net_generation,
  retail_MWh = total_retail) %>% 
  mutate(year = 2019)
```
...
scrape.it <- function(the_year){
  ...
  the_df <- data.frame(
        ...
        capacity_MW = as.numeric(gsub(",", "",the_capacity)),
        netgeneration_MWh = as.numeric(gsub(",", "",net_generation)),
        retail_MWh = as.numeric(gsub(",", "",total_retail))) %>% 
