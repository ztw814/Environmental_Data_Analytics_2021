...
NTL.phys.data.PeterPaul <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) # filter by a list using %in%
...
NTL.phys.data.199099 <- filter(NTL.phys.data, year4 >=1990, year4 <=1999, lakename == "Tuesday Lake")
...
Pipe is another method to wrangle datasets that looks cleaner and is easier to read.  We designate a pipe with `%>%` (ctrl-shift-m). A good way to think about the function of a pipe is with the word "then." 
...
NTL.phys.data.July <- NTL.phys.data %>%
  mutate(month = month(sampledate)) %>% #month function from lubridate
...
NTL.phys.data.surftemp <-
  NTL.phys.data.July %>% 
  filter(depth == 0) %>%
  summarize(mean_surf_temp_C = mean(temperature_C))
...
ex6 <- NTL.phys.data %>% 
  pivot_wider(id_cols = lakeid:sampledate, #columns you want to keep in the new dataset
              names_from = depth, #new column names
              values_from = temperature_C) #new values in each column
