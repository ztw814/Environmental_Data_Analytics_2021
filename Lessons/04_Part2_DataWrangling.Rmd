...
class(NTL.phys.data.PeterPaul$sampledate) #back to character bc we reimported the data from a csv file
...
# Notice that other lake names didn't go away, even though they have zero values; use droplevels to get rid of categories w/ zero values
...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #take dataset
  mutate(month = month(sampledate)) %>% #create month column
  select(lakeid:daynum, month, sampledate:comments) %>% #rearrange month column
...
NTL.phys.data.PeterPaul.subset <- select(NTL.phys.data.PeterPaul, 
                                         lakename:irradianceDeck) #remove lakeid
  
NTL.nutrient.data.PeterPaul.subset <- select(NTL.nutrient.data.PeterPaul, 
                                             lakename, year4, daynum, month, sampledate, depth:po4) #remove lakeid

# write a more succinct line of code to subset the nutrient dataset. 
# or: select(NTL.nutrient.data.PeterPaul, lakename:po4)
```
...
We will work with the new functions from `tidyr`. For gather we will use `pivot_longer` and for spread we will use `pivot_wider`. Note that if you are familiar with `gather` and `spread` you may still use it, they are not going away, but they are not under active development.
...
NTL.nutrient.data.PeterPaul.gathered <- pivot_longer(NTL.nutrient.data.PeterPaul.subset,tn_ug:po4, names_to = "nutrient", values_to = "concentration") #nutrient column will contain nutrient names, concentration column will contain associated values
#gathered dataset should have more rows

#remove NAs in concentration column
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))
count(NTL.nutrient.data.PeterPaul.gathered, nutrient) #for each nutrient variable level, count how many times it appears
...
#specify path to folder where all the files you want are stored
LitterFiles #contains all files
...
LitterTrap <- left_join(Litter, Trap, by = c("siteID", "plotID", "trapID")) #Litter = x, Trap  = y, by = which columns to match
