...
> Answer: check if they have a robots.txt file, terms of service mentioned
...
df_withdrawals <- data.frame("Month" = rep(1:12),
                             "Year" = rep(2000,12), # repeat "2000" 12 times
...
df_withdrawals <- df_withdrawals %>% 
  mutate(Registrant = !!the_registrant, #!! means this is a variable
...
the_year <- 2015
the_scrape_url <- paste0(the_base_url, '/', the_facility, '/', the_year) #paste0 eliminates spaces bt strings
...
scrape.it <- function(the_year, the_facility){
  ...
  #Sys.sleep(1) #uncomment this if you are doing bulk scraping! tells R to pause for 1 second before continuing
...
the_facility_ids <- the_main_website %>% 
  ...
  html_attr("href") %>%           #Pull the "href" attribute from the node (instead of just straight text)
...
dfs_2020 <- map2(the_years, the_facility_ids_subset, scrape.it) #using map2 to iterate through both lists in parallel (first item in both lists, then second item in both lists, etc)

#Conflate the returned list of 20 dataframes into a single one
...
the_df <- cross2(the_years,the_facilities) %>% 
  map(lift(scrape.it)) %>% #lift applies cross2 to map/scrape.it function
  bind_rows()
