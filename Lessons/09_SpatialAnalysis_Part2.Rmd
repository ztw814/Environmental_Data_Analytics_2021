...
st_bbox(counties_sf) #returns coordinates of upper left/lower right corners of box around extent
...
nc_powerplants_sf %>% 
  st_drop_geometry() %>% 
  count(TYPE, sort=TRUE) #shows unique values of TYPE in console
...
svi2018_nc_raw <- read.csv(
  ...
  colClasses = c('FIPS' = 'factor')) %>% #import FIPS column as factor
  select(COUNTY, FIPS, LOCATION, E_TOTPOP, E_POV, E_MINRTY)

#Check structure
str(svi2018_nc_raw)

#Join the SVI attributes to the county spatial features in base R
counties_sf_join <-  merge(x = counties_sf,
                           y = svi2018_nc_raw, 
                           by.x = "GEOID", # join field in x dataset
                           by.y = "FIPS" ) # join field in y dataset
...
enviroatlas_huc12 <- read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv",
                              colClasses = c("HUC_12" = "factor")) %>% 
  mutate(HUC_8 = substr(HUC_12,1,8)) %>% #taking characters 1-8 from HUC_12 to form HUC_8
  group_by(HUC_8) %>% 
...
windenergy_huc8 <- huc8_sf %>% 
  left_join(enviroatlas_huc12, by = c('HUC_8','HUC_8')) #not running idk why
...
#Map the data
mapview(huc8_sf_DWQBasin, zcol = "acres")
...
huc2_utm <-        st_transform(huc8_sf_DWQBasin, crs = 26917)
```
...
neuse_intersect_1 <- counties_sf_utm[neuse_sf,] #finding which counties intersect w/ neuse_sf

#Select intersecting counties using the `filter()` command (Tidyverse)
neuse_intersect_2 <- counties_sf_utm %>% 
  filter(st_intersects(x = ., y = neuse_sf, sparse = FALSE)) #x = . means x = counties_sf_utm (earlier dataset in pipe)
...
neuse_counties_sf <- neuse_sf %>% #cookie cutter
  st_intersection(counties_sf_utm) #cookie dough to cut out of
myMap + mapview(neuse_counties_sf, alpha.regions = 0)

#Update the area of the features (attribute), now that some are clipped
neuse_counties_sf <- neuse_counties_sf %>% 
  mutate(Area_m2 = st_area(neuse_counties_sf$geometry)) %>% #calculating area of each cut off county
  as.numeric() #adding area as numbers not with its units
...
triangle_intersect <- huc8_sf_utm %>% 
  filter(st_intersects(x = ., y = the_triangle, sparse = FALSE))
mapview(the_triangle, alpha.regions = 0, color = "red") + mapview(triangle_intersect)

#Intersect the HUC_8s
triangle_intersection <- huc2_utm %>% 
  st_intersection(the_triangle)
mapview(triangle_intersection) + mapview(the_triangle, alpha.regions = 0, color = "red")
...
huc2_WGS84<- st_transform(huc8_sf_DWQBasin,c=4326)
...
sync(m1,m2) #can also export to html and others can explore, pan, etc.
```
