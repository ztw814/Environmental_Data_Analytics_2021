...
mytheme <- theme_classic(base_size = 14) +
  theme(axUniis.text = element_text(color = "black"), 
        legend.position = "top")
theme_set(mytheme)
```
...
Assumptions: all populations normally distributed, equal variance, independent observations
...
#results: reject null hypothesis i.e. difference between a pair of group means is statistically significant/not all means are equal (but doesn't tell which are different)

# Format ANOVA as lm: shows which plot means are different
...
# Post-hoc test: compare differences between all possible group pairings
# TukeyHSD() computes Tukey Honest Significant Differences, takes an anova object
TukeyHSD(Litter.Totals.anova)

# Extract groupings for pairwise relationships using HSD.test, takes an anova object
Litter.Totals.groups <- HSD.test(Litter.Totals.anova, "plotID", group = TRUE) #gives a list of all variable levels/plots that have the same mean - if you reran the anova all means from these sites would be the same
...
Litter.anova.2way5 <- aov(data = Litter, dryMass ~ Litter.interaction) #comparing means of each dryMass of each interaction
