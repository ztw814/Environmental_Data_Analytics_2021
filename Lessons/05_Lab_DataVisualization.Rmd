...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(x=0,y=0.025,label=expression(paste("Mean is ", x,", ", R^{2},'=0.6'))) # {} means superscript, [] means subscript, paste combines strings, can use different greek letters
print(Plot_text)
...
surf_temp <- ggplot(subset(PeterPaul.chem.nutrients, 
                           depth == 0)) +
  geom_point(aes(x = sampledate, 
                 y = temperature_C, 
                 color = year4)) +
    # could say color=as.factor(year4) so that
    # year isn't visualized as continuous
  facet_wrap(vars(lakename)) + 
    #add nrow=2 to visualize plots on top of each other
  labs(y = "Temperature (C)")
    # instead, could use ylab("Temperature (C)")
print(surf_temp)
...
date_temp <- ggplot(PeterPaul.chem.nutrients) +
  geom_point(aes(x = sampledate, y = temperature_C), color = depth, dotsize = 0.5) +
    # put color/size outside aes() so it doesn't show up in legend
  scale_color_viridis(option = "magma", direction = -1) +
  scale_x_date(date_breaks = "5 years", date_labels = "%Y")
print(date_temp)
    # use scale_x_continuous when variable isn't a date
...
AQI <- ggplot(EPAair, aes(x = PM2.5, y = Ozone, color = SITE_LATITUDE)) +
  geom_point(alpha = 0.5) + 
  geom_smooth(method = lm, color = "red")
    # add se=FALSE to remove standard error shading from line
print(AQI)
...
ggplot(Neonics) +
  ...
  # use scale_x_discrete to scale w/ non-continuous variable
