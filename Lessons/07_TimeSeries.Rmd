...
**Stationary process**: no trend over time - ex white noise series w/ independent, identically distributed random variables
**Autocorrelation function**: measures linear dependency/correlation in observations of a variable along a time-series
**Partial autocorrelation function**: measures linear dependency  structure along a time-series by removing intermediate variables (all observations bt Yt and Yt-h), leaving only the direct correlation bt the two (accounts for possibility that the correlation bt Yt and Yt-1 may be so strong that it makes the relationship bt Yt and Yt-h significant)
**Autoregressive model**: trying to represent time-series based on previous observations/lagged data points (not other variables)
**Unit root**: 
**Trend-stationarity**: detrending a linear trend by running a regression and obtaining the series of residuals, which are the detrended series
**Stochastic trend**: trend that changes in magnitude over time (not perfectly linear w/ constant slope)
...
* **Autocorrelation**: Data points from the same variable are not independent from one another (i.e., the measurement at a given time point is dependent on previous time point(s)) - ex. lagged dataset
...
2. A **trend** component, which quantifies the upward or downward progression over time. The trend component of a time series does not have to be monotonic (not stochastic/varying).
...
```{r}
#transforming variable of interest into a time series
EnoDischarge.late_ts <- ts(EnoDischarge.late$discharge.mean, start = c(1985,1), frequency = 365) #c(year, month)
```

The `stl` function decomposes the time series object into its component parts. We must specify that the window for seasonal extraction is either "periodic" or a specific number of at least 7. The decomposition proceeds through a loess (locally estimated scatterplot smoothing) function. Can only input a univariate time series.

```{r}
?stl
# Generate the decomposition
EnoDischarge.late_Decomposed <- stl(EnoDischarge.late_ts, s.window = "periodic") # can say periodic bc we already specified frequency of dataset in ts(), otherwise could say it here
...
# Must replace, not remove NAs therefore must use na.approx, not drop_na
wind_data_clean <- 
  wind_data %>% 
  mutate( AWND.clean = zoo::na.approx(AWND) ) #zoo::na.approx - calling na.approx function specifically from zoo package; doesn't require loading the entire zoo package first

summary(wind_data_clean$AWND.clean)
#Note the NA is gone, all other stats are the same
...
# Generate time series object (trend test needs ts, not data.frame)
...
# Inspect results
wind_data_trend1 # p<0.001, so reject null that data is stationary, there is a trend
...
# Inspect results
wind_data_trend2
summary(wind_data_trend2) # some seasons have a stronger negative trend
...
> Answer: This seasonal time series has a significant decreasing trend.
