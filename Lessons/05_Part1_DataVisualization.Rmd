...
# Scatterplot (plot dimensions changed above)
...
O3plot2 <- ggplot(EPAair) +
  ...
  geom_point(aes(x = Date, y = Ozone), color = "blue")
  # color must be outside aes definition of x,y
  # adding something inside aes() means adding another layer to the plot/data
print(O3plot2)
...
PMplot <- 
  ...
  # shape = as.factor(Year) means assign a different shape to data from each year
  # color is within aes bc adding another layer - color based on Site.Name
  geom_point()
print(PMplot)
...
PMplot.faceted <-
  ...
  #facet_wrap splitting the one plot into multiple plots based on site; displayed in 3 rows (nrow)
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
                  Site.Name == "Bryson City"), # subset to only include the three sites; use | to include all three sites - CM or L or BC
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) # want separate plots based on site and year (don't need shape = as.factor)
print(PMplot.faceted2)

# Plot true time series with geom_line
PMplot.line <- 
  ggplot(subset(EPAair, Site.Name == "Leggett"), 
         aes(x = Date, y = PM2.5)) + #only including Leggett
  geom_line()
print(PMplot.line)
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #to invert y scale to show depth
print(tempvsdepth)

NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm) + # creating a trendline/curve based on data (lm = linear model/line)
  geom_abline(aes(slope = 16, intercept = 0)) # adding a user-defined line (not based on data)
print(NvsP)
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
# Box and whiskers plot (visualizes point distribution)
Nutrientplot3 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? fill would only fill the box itself, leaving points/outlines black
print(Nutrientplot3)
...
Nutrientplot4 <-
  ...
  #binaxis: stack points on y axis (vertically)
  #stackdir: stack points over the center
  #position: don't overlay points - let them spill out to the side to see all of them
print(Nutrientplot4)

# Violin plot (visualizes density plot in addition to distribution)
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
Nutrientplot6 <-
  ...
  # adding a polygon for each nutrient
...
print(Nutrientplot6) #bad plot: required a lot of code, no legend, x-axis is wrong

# Using a gathered dataset to solve problems from plot6
Nutrientplot7 <-   
  ggplot(PeterPaul.chem.nutrients.gathered) +
  geom_freqpoly(aes(x = concentration, color = nutrient))
print(Nutrientplot7) #works better bc nutrients are all within one column, so can just call the column and ggplot will visualize better
...
Nutrientplot6 <-
  ...
  geom_density_ridges(aes(fill = lakename), alpha = 0.5) # requires additional package; makes a separate ridge for each y value w/ different fill according to lakename
print(Nutrientplot6)
