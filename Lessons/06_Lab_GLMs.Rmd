...
> Answer:
Null: Mean ozone levels are greater than or equal to 50 (mu >= 50)
Alternative: Mean ozone levels are not greater than 50 (mu < 50)
...
shapiro.test((EPAair.subsample$Ozone)) #H0: data are normally dist
...
Ozone.plot <- ggplot(EPAair, aes(x = Ozone)) +
  ...
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0)) #expand gets rid of buffer space b/t graph and axes
print(Ozone.plot)
```
...
> On average, ozone measured in NC is less than 50ppm (t-test, df = 6829, p < 0.0001).
...
#p-value less than 0.05 then reject null for 2018 and 2019 i.e. true ratio not equal to one/variances are not equal
...
O3.twosample #the means are not equal
...
pm25 <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(pm25)
...
pm25.plot <- ggplot(EPAair, aes(x = Ozone, y = PM2.5)) +
  geom_point() +
...
EPA_lat <- lm(data = EPAair, PM2.5 ~ Ozone + SITE_LATITUDE)
summary(EPA_lat)
...
EPA_lm <- lm(data = EPAair, PM2.5 ~ Ozone + SITE_LATITUDE + SITE_LONGITUDE + Month + Year)
step(EPA_lm)
...
EPA_AIC <- lm(data = EPAair, PM2.5 ~ Ozone + SITE_LATITUDE + SITE_LONGITUDE + Month)
summary(EPA_AIC)
