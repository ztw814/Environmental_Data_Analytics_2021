...
O3plot1 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_gray() # nothing happens - theme_gray is default
print(O3plot1)

O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw() #removes gray background, keeps gridlines, black axes
print(O3plot2)

O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light() #no background, all grey gridlines/axes
print(O3plot3)

O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic() #no gridlines/background, only black axes
print(O3plot4)
```

Notice that some aspects of your graph have not been adjusted, including: 

* text size (default size 11)
* axis label colors (default black labels, grey axis text)
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
#defining our own theme by manipulating existing theme
mytheme <- theme_classic(base_size = 14) + #increasing font size
  theme(axis.text = element_text(color = "black"),
        legend.position = "top") #alternative: legend.position + legend.justification (right, left, etc)

### why did label font size change as well?
...
theme_set(mytheme) #set theme across all subsequent ggplots
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  #hline adds a user-defined (h)orizontal line to plot
  geom_hline(yintercept = 50, lty = 2) +
  geom_hline(yintercept = 100, lty = 2) +
  #alpha changes transparency of points
  geom_point(alpha = 0.5, size = 1.5) +
  #geom_text define exact x,y position of text
  ...
  #scaling x axis, date_labels specifies format of date label
  #can only use scale_x_date for dates?
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") +
  # [] makes subscript, * knits two strings together
  ylab(expression("O"[3]* " AQI Value")) +
  #changing x-axis text only
  #use element_text when changing within theme
...
#display all color ramps available w/in a package
...
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth, shape = lakename)) +
    geom_point(alpha = 0.7, size = 2.5) + # add some transparency to points, make bigger
  labs(color = "Depth", shape = "Lake name") + # how to change x/y labs?
  scale_shape_manual(values = c(15, 17)) + # changing shape of points (choosing shapes 15 and 17)
  #scale_color_distiller(palette = "Blues", direction = 1) + # use scale_color_brewer for discrete variables
  scale_color_viridis(option = "magma", direction = -1) + #choosing viridis package, magma ramp w/in package, inverting direction to go from light to dark w/ depth
...
library(cowplot)
plot_grid(NvsP2, Nutrientplot, nrow = 2, align = 'h', rel_heights = c(1.25, 1)) #rel_heights allows the plots to be different heights to fit better
